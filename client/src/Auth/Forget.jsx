                        ...
                        <div>
                            <div className="fill-in">
                                { !enterReset && !enterInputPassword && (
                                    <Form.Field>
                                        <Input type="text" icon="users" iconPosition="left" placeholder="Username"
                                               style={{marginRight: 4 + 'em'}}
                                               onChange={(event) => this.setState({
                                                   username: event.target.value,
                                                   invalidCodeOrPasswordMessage: ''
                                               })}/>
                                        { invalidCodeOrPasswordMessage && <Label basic color="red"
                                                                                 pointing="left">{ invalidCodeOrPasswordMessage }</Label> }
                                    </Form.Field>
                                )}
                                { enterReset && !enterInputPassword && (
                                    <Form.Field>
                                        <Input type="text" icon="code" iconPosition="left"
                                ...
                                { enterInputPassword && (
                                    <Form.Field>
                                        <Input type="password" icon="code" iconPosition="left" placeholder="Password"
                                               style={{marginRight: 4 + 'em'}}
                                               onChange={(event) => this.setState({
                                        ...
                                        { invalidPasswordMessage &&
                                        <Label basic color="red" pointing="left">Password does not match</Label>}
                                        { invalidCodeOrPasswordMessage && <Label basic color="red"
                                                                                 pointing="left">{ invalidCodeOrPasswordMessage }</Label> }
                                    </Form.Field>
                                )}
                            ...
                            <div className="button-holder">
                                { enterReset &&
                                <Button primary fluid onClick={this.enterInputPassword}>Reset password</Button> }
                                { enterInputPassword &&
                                <Button primary fluid onClick={this.handlePasswordReset}>Submit</Button> }
                                { enableSend &&
                                <Button primary fluid onClick={this.sendVerificationCode}>Send Verficiation
                                    Code</Button> }
                                { !enableResend && !enableSend &&
                                <Button fluid loading disabled>Waiting to resend</Button> }
                                { enableResend && enterReset &&
                                <Button fluid color="purple" onClick={ this.sendVerificationCode }>Resend it!</Button> }
                            </div>
                        </div>
                    )}
