                        ...
                        <div>
                            <div className="fill-in">
                                <form>
                                    <Row>
                                        <Form.Field>
                                            <Input type="text" icon="user plus" iconPosition="left"
                                                   placeholder="Username" style={{marginRight: 4 + 'em'}}
                                                   onChange={(event) => this.setState({
                                                       username: event.target.value,
                                                       invalidUserNameMessage: '',
                                                       invalidFormDataMessage: ''
                                                   })}/>
                                            { invalidUserNameMessage && <Label basic color="red"
                                                                               pointing="left">{ invalidUserNameMessage }</Label> }
                                            { username && !this.checkUsernameMatch(username) &&
                                            <Label basic color="red" pointing="left">Invalid username, must conatin
                                                atleast 1 character</Label> }
                                        </Form.Field>
                                    </Row>
                                    <Row>
                                        <Form.Field>
                                            <Input type="password" icon="hashtag" iconPosition="left"
                                                   placeholder="Password" style={{marginRight: 4 + 'em'}}
                                                   onChange={(event) => this.setState({
                                                       password: event.target.value,
                                                       invalidPasswordMessage: '',
                                                       invalidFormDataMessage: ''
                                                   })}/>
                                            { invalidPasswordMessage && <Label basic color="red"
                                                                               pointing="left">{ invalidPasswordMessage }</Label> }
                                        </Form.Field>
                                    </Row>
                                    <Row>
                                        <Form.Field>
                                            <Input type="password" icon="hashtag" iconPosition="left"
                                                   placeholder="Re-enter Password" style={{marginRight: 4 + 'em'}}
                                    ...
                                    <Row>
                                        <Form.Field>
                                            <Input type="email" icon="envelope" iconPosition="left" placeholder="Email"
                                            ...
                                            { invalidEmailMessage &&
                                            <Label basic color="red" pointing="left">{ invalidEmailMessage }</Label> }
                                            { email && !this.checkEmailPattern(email) &&
                                            <Label basic color="red" pointing="left">Invalid email format</Label> }
                                        </Form.Field>
                                    </Row>
                                    <Row>
                                        <Form.Field>
                                            <Input type="tel" icon="phone" iconPosition="left" placeholder="Phone"
                                                   style={{marginRight: 4 + 'em'}}
                                                   onChange={(event) => this.setState({
                                            ...
                                            { invalidPhoneMessage &&
                                            <Label basic color="red" pointing="left">{ invalidPhoneMessage }</Label> }
                                            { phone && !this.checkPhonePattern(phone) &&
                                            <Label basic color="red" pointing="left">Valid format:
                                                +11234567890</Label> }
                                        </Form.Field>
                                    </Row>
                                </form>
                            </div>
                            <div className="button-holder"><Button primary fluid
                                                                   onClick={this.handleSubmit}>Register</Button>
                                { invalidFormDataMessage &&
                                <Label basic color="red" pointing="left">{ invalidFormDataMessage }</Label> }
                            </div>
                        </div>
                    )}
                    {this.state.enterAuth && !this.state.authSuccess && (
                        <div>
                            <div className="fill-in">
                                <Row>
                                    <Input type="text" icon="unlock alternate" iconPosition="left"
                                           placeholder="Verification Code" style={{marginRight: 4 + 'em'}}
                                           onChange={(event) => this.setState({
                                               code: event.target.value,
                                               invalidCodeMessage: ''
                                           })}/>
                                    { invalidCodeMessage &&
                                    <Label basic color="red" pointing="left">Invalid verfication code</Label> }
                                </Row>
                            </div>
                            <div className="button-holder">
                                <Button primary fluid onClick={ this.handleSubmitVerification }>Confirm</Button>
                                { !enableResend &&
                                <Button fluid loading onClick={this.countDownResendVerificationCode()}>Waiting to
                                    resend</Button> }
                                { enableResend &&
                                <Button fluid color="purple" onClick={this.resendVerificationCode}>Resend it!</Button> }
                            </div>
                        </div>
                    )}
